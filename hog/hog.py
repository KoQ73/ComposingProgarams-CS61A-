...
def roll_dice(num_rolls, dice=six_sided):
    ...
    return 1 if rolled_a_one else points
...
def perfect_square(score):
    return True if find_square(score) else False
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    i, max_dice, max_score = 1, 1, 0
    averaged_dice = make_averaged(roll_dice, total_samples)
    while i <= 10:
        max_average_score = averaged_dice(i, dice)
        if max_average_score > max_score:
            max_score = max_average_score
            max_dice = i
        i += 1
    return max_dice
    # END PROBLEM 9
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    tail_points = square_update(0, score, opponent_score)
    difference = abs(score - tail_points)
...
def final_strategy(score, opponent_score):
    ...
    Calculate the mass scoring num_rolls and average score of that.
    Compare it with tail points and play the better one.
    If the score is very far apart, take more risk.
    Take the chance of perfect square every time if its greater than 4.
    Calculate the difference between score and goal to using dice 0.
    *** YOUR DESCRIPTION HERE ***
    ...
    def calculate_risk(score, opponent_score):
        ...
        if difference > 30:
            ...
            return 0
    new_max_roll = max_roll + calculate_risk(score, opponent_score)
    if new_max_roll > 10:
        new_max_roll = 10
    if new_max_roll != max_roll:
        return new_max_roll

    if not square_strategy(score, opponent_score, 9, max_roll):
        return 0

    if tail_strategy(score, opponent_score, max_average_score, max_roll) == 0:
        return 0
