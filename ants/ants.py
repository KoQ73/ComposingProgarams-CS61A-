...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not current.is_hive:
            bee = random_bee(current.bees)
            if bee and self.lower_bound <= counter <= self.upper_bound:
                return bee
            else:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.is_doubled:
            damage *= 2
        bees = list(current.bees)
        for bee in bees:
            bee.reduce_health(damage)
        left_bees = []
        for bee in bees:
            if bee.health > 0:
                left_bees.append(bee)
        current.bees = left_bees
        # END Problem 5
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        # add ants that need to be doubled
        while place:
            if place.ant:
                ants.append(place.ant)
            place = place.exit
        # double the damage of the ants making sure to also doubled the ants that are contained inside an ant_container
        for ant in ants:
            if ant.is_container:
                ant.double()
