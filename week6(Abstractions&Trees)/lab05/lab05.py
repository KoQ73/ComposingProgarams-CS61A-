def flatten(s):
    ...
    """
    "*** YOUR CODE HERE ***"
    # flatten every element of the list and add it to the list
    flattend = []
    for element in s:
        if type(element) == list:
            flattend += flatten(element)
        else:
            flattend += [element]
    return flattend
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    start = make_city('s', lat, lon)
    closer = city_b
    distance1 = distance(start, city_a)
    distance2 = distance(start, city_b)
