...
def eval_and(operands):
    ...
    """
    # BEGIN SOLUTION Q4
    # need calc_eval in here
    if operands == nil:
        return True    
    while operands.rest != nil:
        expr = calc_eval(Pair("and", Pair(operands.first, nil)))
        if expr is 0:
...
def eval_define(expr):
    ...
    """
    # BEGIN SOLUTION Q5
    value = expr.rest.first
    name = expr.first
    if value in bindings:
        bindings[name] = calc_eval(value)
    else:
        bindings[name] = value
    return name
